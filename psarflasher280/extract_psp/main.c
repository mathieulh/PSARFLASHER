...
extern u8 dataPSAR[15100000] __attribute__((aligned(64)));
extern u8 dataOut[2000000] __attribute__((aligned(64)));
...
#endif
{
	...
	fd = sceIoOpen("ms0:/EBOOT.PBP", PSP_O_RDONLY, 0777);
	if (fd < 0)
	{
		ErrorExit(6000, "Cannot find EBOOT.PBP : %08X.\n", fd);
	}
	...
	if (sceIoRead(fd, dataPSAR, size) < size)
	{
	...
	size = decrypt_prx(dataPSAR, size, dataOut);
