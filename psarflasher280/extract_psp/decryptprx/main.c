...
int (*semaphore_4C537C72_k)(void *inbuf, SceSize insize, void *outbuf, int outsize, int cmd);
...
static void ExtraV2Mangle(u8* buffer1, u8 codeExtra)
{
    ...
    int ret = semaphore_4C537C72_k(buffer2, 20+0xA0, buffer2, 20+0xA0, 7);
    if (ret != 0)
...
int DecryptPRX(const u8* pbIn, u8* pbOut, int cbTotal, u32 tag)
{
    ...
    ret = semaphore_4C537C72_k(pbOut+0x2C, 20+0x70, pbOut+0x2C, 20+0x70, 7);
    if (ret != 0)
    {
    ...
    ret = semaphore_4C537C72_k(pbOut, cbTotal, pbOut+0x40, cbTotal-0x40, 0x1);
    if (ret != 0)
    {
...
int decrypt_prx(u8 *inbuf, int insize, u8 *outbuf)
{
	...
	semaphore_4C537C72_k = (void *)FindProc("sceMemlmd", "semaphore", 0x4c537c72);
